...
def DivideFlux(unfolded,is_mc):
    ...
    for pdg in nuPDGs:
        frw= PlotUtils.flux_reweighter(FLUX,pdg,USE_NUE_CONSTRAINT) #playlist is dummy for now
        addedflux = frw.GetIntegratedFluxReweighted(pdg,unfolded,NEUTRINO_ENERGY_RANGE[0],NEUTRINO_ENERGY_RANGE[1],False)
        addedflux.Scale(1e-4 *(mc_pot if is_mc else data_pot)) #change unit to nu/cm^2
        flux.Add(addedflux)
    print("flux=",flux.Print("All"))
...
pion_recovered_err.Draw("E0")
pion_sim_err.Draw("SAME E0")
pion_legend = ROOT.gPad.BuildLegend(0.5,0.7,0.8,0.85)
