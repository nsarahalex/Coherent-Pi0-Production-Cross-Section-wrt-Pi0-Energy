...
def createTarball(outDir):
  ...
  if(not found):
    cmd = "tar -czf %s -C %s %s"%(outDir, baseDir+"../", "{}".format(MacroName))
...
def submitJob( tupleName):
  ...
  #cmd = "jobsub_submit --group=minerva --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --role=Analysis --memory %dMB --tar_file_name dropbox://%s -d HISTS %s -d LOGS %s -N %d --expected-lifetime=%dh  file://%s/%s" % ( memory , outdir_tarball , outdir_hists , outdir_logs , njobs, 24, os.environ["PWD"] , wrapper_name )
  #cmd += "--append_condor_requirements='(regexp(\".*fnpc7.*\",Machine) == FALSE)'"
  cmd = "jobsub_submit --group=minerva --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --append_condor_requirements='(regexp(\".*fnpc7.*\",Machine) == FALSE)' --role=Analysis --memory %dMB -f %s -d HISTS %s -d LOGS %s -N %d --expected-lifetime=%dh  file://%s/%s" % ( memory , outdir_tarball , outdir_hists , outdir_logs , njobs, 36, os.environ["PWD"] , wrapper_name )
...
if __name__ == '__main__':
  ...
  processingID = '%s_%s-%s' % ("NCCoh_selection", dt.date.today() , dt.datetime.today().strftime("%H%M%S") )
  outdir_hists = "/pnfs/minerva/scratch/users/%s/%s_hists" % (os.environ["USER"],processingID)
  ...
  outdir_tarball=gridargs.tarball if gridargs.tarball else "/pnfs/minerva/scratch/users/%s/tarballs/%s-%s.tar.gz" % (os.environ["USER"],os.environ["USER"],processingID)
  ...
  for playlist in gridargs.playlists:
    for dataSwitch in ["mc","data"]:
      ...
      cmdString = "NCCoh-%s-%s" % (playlist,dataSwitch)
      submitJob(cmdString)
