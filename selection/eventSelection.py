...
def plotRecoKin(mc, chainwrapper, outfile):
    ...
    for counter in range(nEvents):
        ...
        for universe in chain.from_iterable(iter(universes.values())):
            ...
            if universe.ShortName() =="cv" and eventClassifier.side_band is not None:
                event = eventClassifier.tree
                #if eventClassifier.side_band == "Signal":
                #    #print("************************RECO RECO RECO*****************************************")
                #    #print(eventClassifier.side_band)
                #    if eventClassifier.truth_class =="NCCohPi0":
                #        #print(eventClassifier.truth_class, "event number = ", event.GetEntry())
                #        #print("ev_run=",event.ev_run, "ev_subrun=",event.ev_subrun,"ev_gate=",event.ev_gate ) #"mc_run=", event.mc_run, "mc_subrun=", event.mc_subrun,"eventID=",event.eventID)
