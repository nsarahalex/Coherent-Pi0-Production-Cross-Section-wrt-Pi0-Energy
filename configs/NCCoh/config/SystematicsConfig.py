...
AnaNuPDG= 14   #-14 for RHC, 14 for FHC
...
NonResPi=True
LowQ2PiWeightChannel = "Joint"  #"MENU1PI" #"Joint"
LowQ2PiWeightSysChannel = [None]
NumZExpansionUniverses = 0 #0 #Means Don't use Zexpansion. 100 is default Z expansion
...
GENIE_ERROR_GROUPS = {
    "GENIE" : ["GENIE_"+ i for i in (GENIE_UNIVERSES+["MaCCQE", "Rvn1pi", "Rvp1pi","MaZExpCCQE"] ) if not (i.startswith("Fr") or i.startswith("MFP")) ]
}
...
CONSOLIDATED_ERROR_GROUPS_CONFIG = {
    #"DETECTOR_RESPONSE_ERROR_GROUPS": [DETECTOR_RESPONSE_ERROR_GROUPS],
    #"GEANT_ERROR_GROUPS" : [GEANT_ERROR_GROUPS],
    "fsi_weight": [ {"fsi_weight": ["fsi_weight"]} ],
    "flux": [ {"flux": ["Flux"]} ],
    ...
    "Eavail_Sys": [ {"Eavail_Sys": ["Eavail_Sys"]} ],
