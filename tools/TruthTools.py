...
def MostEnergeticParticle(event,PDG,anti_particle=True): 
    ...
    if len(particle_index) == 0 :
        ...
        #print([(i, event.mc_FSPartE[i]) for i in particle_index])
        m = max(event.mc_FSPartE[i] for i in particle_index)
        
        #print("MostEnergeticParticle=",m,event.mc_FSPartE.index(m))
        #print("m=",m, "event.mc_FSPartE.index(m)=",event.mc_FSPartE.index(m))
        return event.mc_FSPartE.index(m)
    else : 
        #print("MostEnergeticParticle2=",particle_index[0])
...
#    try:
#        print(f"Index: {index}")
#        print(f"Px: {event.mc_FSPartPx[index]}")
#        print(f"Py: {event.mc_FSPartPy[index]}")
#        print(f"Pz: {event.mc_FSPartPz[index]}")
#        print(f"E : {event.mc_FSPartE[index]}")
#        print(f"RotationX: {ROOT.Math.RotationX(math.radians(-3.3))}")
#        
#        vec = ROOT.Math.PxPyPzEVector(event.mc_FSPartPx[index], event.mc_FSPartPy[index], event.mc_FSPartPz[index], event.mc_FSPartE[index])
#        rotated = ROOT.Math.RotationX(math.radians(-3.3)) * vec
#        
#        return math.degrees(rotated.Theta())
#    
#    except Exception as e:
#        print(f"Error in FindThetaFromIndex: {e}")
#        return None
...
def FindThetaFromIndex(event,index):
    if index is None:
        return None
    #print("index=",index)
    #print("Px=",event.mc_FSPartPx)
    #print("Py=",event.mc_FSPartPy)
    #print("Pz=",event.mc_FSPartPz)
    #print("E=",event.mc_FSPartE)
    if (
        index < len(event.mc_FSPartPx) and
        index < len(event.mc_FSPartPy) and
        index < len(event.mc_FSPartPz) and
        index < len(event.mc_FSPartE)
    ):
...
def Trans(event):
    ...
    if len(event.prong_axis_vector) != 0:
            ...
            recoX = event.MasterAnaDev_vtx[0]/1e3
            recoY = event.MasterAnaDev_vtx[1]/1e3
            recoZ = event.MasterAnaDev_vtx[2]/1e3
...
def Rebin(event): 
    #print("IUE=",event.UpstreamInlineEnergy)
...
def RebinWeightPos(event):
    print("IUE wtdposmean =", event.UpstreamInlineEnergyWgtdPosMean)
...
def PiZeroE_diff(event):
    if  event.mc_current == 2 and event.mc_intType ==10:
        types = event.mc_FSPartPDG
        energies = event.mc_FSPartE
        #print("NCDiffE=",energies)
        for i, val in enumerate(types):
            if val == 111:
                energy = energies[i]
                return energy/1e3
#nimmy defined PiZeroE_Coh similar to PiZeroE_diff

#def PiZeroE_Coh(event):
#    if event.mc_intType ==4:
#        types = event.mc_FSPartPDG
#        energies = event.mc_FSPartE
#        for i, val in enumerate(types):
#            if val == 111:
#                energy = energies[i]
#                #print("PiZeroE_Coh=",energy/1e3)
#                return energy/1e3

def PiZeroE_Coh(event):  #PiZeroE_NCCoh
    if event.mc_intType ==4 and event.mc_current == 2:
        types = event.mc_FSPartPDG
        energies = event.mc_FSPartE
        for i, val in enumerate(types):
            if val == 111:
...
def PiZeroE_NCdiff(event):  #PiZeroE_NCCoh
    if event.mc_intType ==10 and event.mc_current == 2:
        types = event.mc_FSPartPDG
        energies = event.mc_FSPartE
        for i, val in enumerate(types):
            if val == 111:
...
def CosPiZeroTheta(event):
    types = event.mc_FSPartPDG
    for i, val in enumerate(types):
        if val == 111:
            return math.cos(math.radians(FindThetaFromIndex(event,i)))

def LacknerVariable(event):
    pizeroE = PiZeroE(event)
    if pizeroE == None:
        return None
    costheta = CosPiZeroTheta(event)
    if costheta == None:
        return None
    return pizeroE*(1-costheta)

def LacknerRatio(event):
    reco_E_lep = event.kin_cal.reco_E_lep
    reco_cos_theta_lep = event.kin_cal.reco_E_lep
    lackner_variable = LacknerVariable(event)
    if None in [reco_E_lep,reco_cos_theta_lep,lackner_variable]:
        return None
    else: return reco_E_lep*(1-reco_cos_theta_lep)/lackner_variable - 1
