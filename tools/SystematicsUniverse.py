...
from tools.NimmyWeights import GetAlexWeights
...
#        super(CVUniverse,self).SetEntry(n_entry)
    ...
    def GetStandardWeight(self):
        weight = 1.0
        #print("universe",self.ShortName())
        #print("weights for entry",self.GetEntry())

        weight *= self.GetGenieWeight()
        #print("with GetGenieWeight()",self.GetGenieWeight())
        weight *= self.GetFluxAndCVWeight()
        #print("with GetFluxAndCVWeight()",self.GetFluxAndCVWeight())
        weight *= self.GetLowRecoil2p2hWeight()
        #print("with GetLowRecoil2p2hWeight()",self.GetLowRecoil2p2hWeight())
        weight *= self.GetRPAWeight()
        #print("with GetRPAWeight()",self.GetRPAWeight())
        weight *= self.GetMyLowQ2PiWeight()
        #print("with GetMyLowQ2PiWeight()",self.GetMyLowQ2PiWeight())
        weight *= self.GetGeantHadronWeight() # Disable if doing closure test
        #print("with GetGeantHadronWeight()",self.GetGeantHadronWeight())
#        weight *= self.GetAlexWeight() # Disable if doing closure test
    ...
    #def GetEAvail(self):
    #    if self.GetAnaToolName() == "MasterAnaDev":
    #        idx = [i for i in range(len(self.MasterAnaDev_hadron_isSideECAL))if self.MasterAnaDev_hadron_isSideECAL[i] == 1 and self.MasterAnaDev_hadron_isODMatch[i] == 1]
    #        if idx:
    #            energy_tracker = sum(self.part_response_container_total_recoil_passive_primaryHadrons_tracker[i] for i in idx)
    #            energy_ecal = sum(self.part_response_container_total_recoil_passive_primaryHadrons_ecal[i] for i in idx)
    #            total_energy = energy_tracker + energy_ecal
    #            FinalE = self.blob_recoil_E_tracker+self.blob_recoil_E_ecal - total_energy
    #            return self.blob_recoil_E_tracker+self.blob_recoil_E_ecal - total_energy
    #        else:
    #            return self.blob_recoil_E_tracker+self.blob_recoil_E_ecal
    #    else:
    #        return self.recoile_passive_tracker+self.recoile_passive_ecal
...
class CVSystematicUniverse(ROOT.PythonMinervaUniverse, CVUniverse):
...
class FluxUniverse( ROOT.PlotUtils.FluxUniverse(ROOT.PythonMinervaUniverse),CVUniverse):
...
class GenieUniverse(ROOT.PlotUtils.GenieUniverse(ROOT.PythonMinervaUniverse), CVUniverse):
...
class GenieRvx1piUniverse(ROOT.PlotUtils.GenieRvx1piUniverse(ROOT.PythonMinervaUniverse),CVUniverse):
    def __init__(self,chain,nsigma,universe_name):
...
class GenieFaCCQEUniverse(ROOT.PlotUtils.GenieFaCCQEUniverse(ROOT.PythonMinervaUniverse),CVUniverse):
...
class Universe2p2h(ROOT.PlotUtils.Universe2p2h(ROOT.PythonMinervaUniverse), CVUniverse):
    def __init__(self,chain,universe_number):
...
class RPAUniverse(ROOT.PlotUtils.RPAUniverse(ROOT.PythonMinervaUniverse), CVUniverse):
...
class ResponseUniverse(ROOT.PlotUtils.ResponseUniverse(ROOT.PythonMinervaUniverse), CVUniverse):
...
class LowQ2PionUniverse(ROOT.PlotUtils.LowQ2PionUniverse(ROOT.PythonMinervaUniverse),CVUniverse):
...
class LowQ2PionUniverseAlt(CVUniverse):
...
class MuonUniverseMinerva(ROOT.PlotUtils.MuonUniverseMinerva(ROOT.PythonMinervaUniverse),CVUniverse):
    def __init__(self,chain,nsigma):
...
class MuonUniverseMinos(ROOT.PlotUtils.MuonUniverseMinos(ROOT.PythonMinervaUniverse),CVUniverse):
    def __init__(self,chain,nsigma):
...
class GeantHadronUniverse(ROOT.PlotUtils.GeantHadronUniverse(ROOT.PythonMinervaUniverse), CVUniverse):
...
class TargetMassUniverse(ROOT.PlotUtils.TargetMassScintillatorUniverse(ROOT.PythonMinervaUniverse),CVUniverse):
    def __init__(self,chain,nsigma):
...
class BkgTuneUniverse(CVSystematicUniverse):
...
def GetAllSystematicsUniverses(chain,is_data,is_pc =False,exclude=None,playlist=None):
    ...
    if is_data:
        ...
        #Set NonResPi weight
        CVSystematicUniverse.SetNonResPiReweight(SystematicsConfig.NonResPi)
        CVSystematicUniverse.SetDeuteriumGeniePiTune(False)
...
#    path = "/pnfs/minerva/persistent/DataPreservation/p4/FullDetector/Merged_mc_ana_me1A_DualVertex_p4/MasterAnaDev_mc_AnaTuple_run00110000_Playlist.root"
