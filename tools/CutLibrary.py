...
class SelectionCut(object):
    ...
    def DoesEventPass(self, event,nprong=0):
        #print("prong=", nprong, "PDG=", event.MasterAnaDev_hadron_isExiting[nprong])
...
REQUIRE_ZERO_INT = lambda val: int(val) == 0
...
def PrintStuff1(event):
    print(event.GetEntry(),"Eavail_reco",event.kin_cal.reco_visE,"Eavail_true",event.kin_cal.true_visE) #,bool(IsCC(event) and IsPi0InFinalState(event) and IsSignalVisE(event)), event.mc_incoming, event.GetWeight())
    return True
...
#nimmy added Nimmy Pion Energy Cut 
    "Nimmy Pion Energy Cut": {
        "value_getter": lambda event,nprong: TruthTools.PiZeroE(event) if (event.mc_intType==4) else None,
        #"value_getter": lambda event,nprong: TruthTools.PiZeroE_Coh(event),
        "cut_fn": lambda val: val >= CutConfig.PION_ENERGY_CUT if val is not None else True,
    },
    ...
    "MasterAnaDev_hadron_number": {
        "cut_fn": REQUIRE_ZERO_INT,
        "variable_range": VARIABLE_RANGE_01,
    },
    #"Muon_is_not_Exiting": {
    #    "value_getter": lambda event, nprong:(event.MasterAnaDev_hadron_isSideECAL[nprong], event.MasterAnaDev_hadron_isODMatch[nprong]),
    #    "cut_fn": lambda val : (val[0]==0 and val[1] == 0),
    #},

    #"Muon_is_Exiting": { 
    #    "value_getter": lambda event, nprong:(event.MasterAnaDev_hadron_isSideECAL[nprong], event.MasterAnaDev_hadron_isODMatch[nprong]),
    #    "cut_fn": lambda val : (val[0]==1 and val[1] == 1),
    #},
    "Muon_is_not_Exiting": {
        "value_getter": lambda event, nprong=None: [(event.MasterAnaDev_hadron_isSideECAL[i], event.MasterAnaDev_hadron_isODMatch[i]) for i in range(event.n_prongs)],
        "cut_fn": lambda vals: all(not (val[0] == 1 or val[1] == 1) for val in vals), #condition was and
    },

    "Muon_is_Exiting": {
        "value_getter": lambda event, nprong=None: [(event.MasterAnaDev_hadron_isSideECAL[i], event.MasterAnaDev_hadron_isODMatch[i]) for i in range(event.n_prongs)],
        "cut_fn": lambda vals: any(val[0] == 1 or val[1] == 1 for val in vals),  #condition was and
    },
...
#Nimmy modified TrueLeptonEnergy and TrueLeptonAngle for NCCoh analysis
    "TrueLeptonEnergy": {
        "value_getter": lambda event, nprong: event.mc_FSPartE[TruthTools.MostEnergeticParticle(event, 111)] / 1e3 if TruthTools.MostEnergeticParticle(event, 111) is not None else None,
        #"value_getter": lambda event,nprong: event.mc_FSPartE[TruthTools.MostEnergeticParticle(event,111)]/1e3 ,
        #"value_getter": lambda event,nprong: event.kin_cal.true_E_lep,
        "cut_fn": lambda val: CutConfig.ELECTRON_ENERGY_RANGE[0] <= val < CutConfig.ELECTRON_ENERGY_RANGE[1] if val is not None else True,
    },
    
    "TrueLeptonAngle": {
        "value_getter":lambda event,nprong: TruthTools.FindThetaFromIndex(event,TruthTools.MostEnergeticParticle(event,111)) ,
        #"value_getter": lambda event,nprong: event.kin_cal.true_theta_lep,
        "cut_fn": lambda val: CutConfig.ELECTRON_ANGLE_RANGE[0] <= val < CutConfig.ELECTRON_ANGLE_RANGE[1] if val is not None else True,
    },
