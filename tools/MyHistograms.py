...
#3D nimmy added
class HistWrapper3D(HistWrapper):
    def __init__(self, title, Xbins, Ybins, Zbins):
        # Note: bins are arrays of lower edges for each axis
        # We pass 3 sets of bin counts and arrays to the parent HistWrapper
        super(HistWrapper3D, self).__init__(
            title,
...
class PlotProcessor():
    def __init__(self,name,title,binning,value_getter = None, cuts = None, weight_function = None,**kwargs):
        ...
        elif len (value_getter) == 3:
            self.histwrapper = HistWrapper3D(title,binning[0],binning[1],binning[2])  #nimmy added 3D
