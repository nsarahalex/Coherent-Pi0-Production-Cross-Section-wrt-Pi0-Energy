...
print("SelectionFilesRegex=",SelectionFilesRegex)
...
def AddOneFile(input_string,output_string, pot_scale):
    ...
    for key in keylist:
        hist = input_file.Get(key.GetName())
        #hist.SetDirectory(None)
        if hist is None or isinstance(hist,ROOT.TTree):
            continue
        print(hist)
        if hist:
            hist.Scale(pot_scale)
...
def AddRegexMatchedFiles(dir_path,f = None):
    ...
    print(f"Using regex: {SelectionFilesRegex}\n")
    for string in files:
